header <- dashboardHeader(title = "Spotify Analytics")
...
sidebar <- dashboardSidebar(
  sidebarMenu(
    ...
    menuItem("About Me", icon = icon("linkedin"), href = "https://www.linkedin.com/in/frengky-langgeng-83b391172/")
  )
)
...
body <- dashboardBody(
  tabItems(
    ...
    tabItem(tabName = "overview",
            ...
            fluidRow(
              valueBox("144 Million", "Subscribers", icon = icon("play-circle"), color = "black"),
              valueBox("320 Million", "Monthly active users", icon = icon("users"), color = "green"),
              valueBox("92", "Markets", icon = icon("globe-asia"), color = "black"),
              valueBox("60 Million+", "Tracks", icon = icon("music"), color = "green"),
              valueBox("1.9 Million+", "Podcasts", icon = icon("podcast"), color = "black"),
    ...
    tabItem(tabName = "stats", 
            ...
            fluidPage(
              box(width = 8,
                  solidHeader = T,
                  title = tags$b("Music Taste Trends"), 
                  plotlyOutput(outputId = "time_series")
              )),
            fluidPage(
              box(width = 8,
                  solidHeader = T,
                  title= tags$b("Top 10 Genres all of time"),
                  plotlyOutput("top_genre"))),
