...
function(input, output){
  ...
  output$top_genre <- renderPlotly({
    data_agg1 <- genre %>% 
      select(genres, counts) %>% 
      arrange(desc(counts)) %>% 
      mutate(label = glue("genres {genres}
                      counts: {counts}")) %>%
      head(10)
    plot_genre <- ggplot(data_agg1, aes(x = counts, y = reorder(genres, counts), text = label)) +
      geom_col(aes(fill = counts)) +
      scale_fill_gradient(low="red", high = "black") +
      labs(x = "Popularity (0-100)", y = NULL, 
           title = glue("Top 10 Genres")) +
      my_theme
